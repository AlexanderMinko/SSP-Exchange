...
- Produces: proof the wire contract works: 200 fill with substituted `adm`/`nurl` in the body, 204, 403, 400.
...
2. `docker compose up -d` → `./mvnw spring-boot:run` → smoke test:
   ...
   ```
   Expect **204** (the seeded bidder endpoints are unreachable until the dummy DSP exists) and exactly one `AUCTION_EVENTS` JSON line on stdout showing `"outcome":"ERROR"` or `"TIMEOUT"` per bidder. A 403/400 means a config/seed problem.
